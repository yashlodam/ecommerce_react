...
function OrderDetails() {
  ...
  // Ensure currentOrder in Redux matches the requested orderId in URL
  ...
  // Loading state
  if (isLoading || (!isMatchingOrder && !order?.error)) {
    return (
  ...
  if (order?.error || !currentOrder) {
    return (
      <div className="max-w-md mx-auto text-center py-16 px-4 space-y-4">
        <div className="w-14 h-14 rounded-full bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 flex items-center justify-center mx-auto text-rose-500">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          {typeof order?.error === "string"
            ? order.error
            : "We couldn't retrieve the details for this order. It may have been moved or you might not have permission to view it."}
        </p>
        <div className="pt-2">
          <Button
            variant="contained"
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-6 px-1 sm:px-4 py-2 sm:py-6">
      {actionMsg.text && (
        <Alert
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 sm:mb-6">
          Real-time fulfillment tracking for this order
        </p>
        ...
        <div className="space-y-3">
          {items.map((item) => {
            ...
            const isSelectedItem =
              orderItemId && String(item.id) === String(orderItemId);

            return (
              <div
                key={item.id}
                className={`bg-white dark:bg-slate-900 border rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm transition-all ${
                  isSelectedItem
              ...
              >
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-start sm:items-center">
                  <div className="w-20 h-20 sm:w-28 sm:h-28 rounded-xl sm:rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 shrink-0 flex items-center justify-center p-2">
                    <img
                  ...
                  <div className="flex-1 space-y-1.5 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isSelectedItem && (
                        <span className="text-[10px] font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/60 px-2 py-0.5 rounded-md border border-teal-200 dark:border-teal-800">
                          Selected Item
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 sm:gap-4 text-xs text-slate-500 dark:text-slate-400">
                      <p>
                        ...
                        <span className="text-slate-800 dark:text-slate-200">
                          {item?.size || product?.sizes || "Standard"}
                        </span>
                      </p>
                      <p>
                      ...
                      <p>
                        ...
                        <span className="text-teal-700 dark:text-teal-400 font-extrabold">
                          {formatINR((item?.sellingPrice || 0) * (item?.quantity || 1))}
                        </span>
                      </p>
                    </div>
                    ...
                    {item?.appliedDealTitle && (
                      <p className="text-xs font-bold text-teal-600 dark:text-teal-400 flex items-center gap-1 pt-1">
                        <span>🔥</span>
                        <span>Promotional Deal: {item.appliedDealTitle}</span>
                        {item.discountAmount > 0 && (
                          <span className="text-slate-500 font-normal">
                            (-{formatINR(item.discountAmount)} discount)
                          </span>
                        )}
                      </p>
                    )}
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-slate-900 dark:text-slate-100">
                {currentOrder?.totalItems || items.reduce((acc, i) => acc + (i.quantity || 1), 0)}
              </span>
            </div>
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
          ...
          {currentOrder?.shippingAddress ? (
            <div className="text-sm text-slate-700 dark:text-slate-300 space-y-1">
              ...
              <p>{currentOrder.shippingAddress.address || "Address"}</p>
              <p>
                {[
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ fontSize: "0.875rem", color: "text.secondary" }}>
            Are you sure you want to cancel this order? Any reserved product
            stock will immediately be restored. If you paid online via Razorpay,
